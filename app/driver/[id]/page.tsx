...
function TrackMapPanel({
  ...
  setSelectedCornerId: (value: number | null) => void;
  incidentMarkers: IncidentMarker[];
  selectedIncidentId: number | null;
  setSelectedIncidentId: (value: number | null) => void;
  addMarkerMode: boolean;
  setAddMarkerMode: (value: boolean | ((prev: boolean) => boolean)) => void;
  setIncidentMarkers: React.Dispatch<React.SetStateAction<IncidentMarker[]>>;
  setMapAspectRatio: (value: number) => void;
  mapAspectRatio: number;
}) {
  return (
    <section className="rounded-[28px] border border-[#2A3441] bg-[#141A22] p-5 shadow-2xl md:p-7">
      <div className="mb-5 flex flex-col gap-4 border-b border-[#2A3441] pb-5 xl:flex-row xl:items-center xl:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-white">Track Overview</h2>
          <p className="mt-2 text-sm text-[#9CA3AF]">
            This uses the same track map and annotations style as the PDF overview page.
          </p>
        </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.45fr_0.95fr]">
        ...
        <div className="rounded-[24px] border border-[#2A3441] bg-[#111827] p-4">
          <h3 className="text-lg font-semibold text-white">Map Legend</h3>
          <div className="mt-4 space-y-3 text-sm text-[#C7D0D9]">
            <div className="flex items-center gap-3">
              <span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-[#E10600] px-2 text-xs font-bold text-white">
                T
              </span>
              <span>Corner marker linked to Entry / Mid / Exit balance.</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-yellow-500 bg-[#141A22] text-base text-white">
                🪖
              </span>
              <span>Free-form incident marker that appears on the map and PDF.</span>
            </div>
          </div>

          <div className="mt-6">
            <h4 className="text-sm font-semibold uppercase tracking-[0.16em] text-[#9CA3AF]">
              Corner Summary
            </h4>
              ...
              {template.corners.map((corner) => (
                <button
                  ...
                  className={`w-full rounded-2xl border p-3 text-left transition ${
                    selectedCornerId === corner.id
                      ? "border-[#E10600] bg-[#1B2430]"
                      : "border-[#2A3441] bg-[#141A22] hover:border-[#3A4758]"
                  }`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-white">T{corner.id}</div>
                    </div>
                  </div>
                </button>
              ))}
...
export default function DriverTemplatePage() {
  ...
  const completedCornerIds = useMemo(() => {
    return new Set(
      cornerFeedback
        .filter(
          (entry) =>
  ...
  async function handleSendPdf() {
    ...
    try {
      ...
      const payload = {
        ...
        sessionSlug: slugify(sessionName),
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#0A0E14] px-4 py-8 text-white">
        <div className="mx-auto max-w-6xl">Loading debrief…</div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#0A0E14] px-4 py-6 text-white md:px-8 md:py-8">
      <div className="mx-auto max-w-6xl space-y-5 md:space-y-6">
        <section className="rounded-[28px] border border-[#2A3441] bg-[#141A22] px-5 py-6 shadow-2xl md:px-8 md:py-7">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              <div className="rounded-2xl border border-[#2A3441] bg-[#1B2430] px-4 py-3">
                <div className="text-xs uppercase tracking-[0.16em] text-[#9CA3AF]">
                  Corners completed
                </div>
                <div className="mt-1 text-lg font-semibold text-white">
                  {completedCorners}/{template.corner_count}
                </div>
              </div>
              <div className="rounded-2xl border border-[#2A3441] bg-[#1B2430] px-4 py-3">
                <div className="text-xs uppercase tracking-[0.16em] text-[#9CA3AF]">
              ...
              <div className="rounded-2xl border border-[#2A3441] bg-[#1B2430] px-4 py-3">
                ...
                <div className="mt-1 text-sm font-semibold text-white break-all">
                  {template.track_name}_{driverName || "driver"}_{sessionName || "session"}.pdf
                </div>
              </div>
            </div>
          </div>
        ...
        <TrackMapPanel
          ...
          setIncidentMarkers={setIncidentMarkers}
          setMapAspectRatio={setMapAspectRatio}
        ...
        <section className="rounded-[28px] border border-[#2A3441] bg-[#141A22] p-5 shadow-2xl md:p-7">
          ...
          <p className="mt-2 text-sm text-[#9CA3AF]">
            Choose the main engineer, and optionally add one extra recipient such as the team
            manager.
          </p>
            ...
            {!showExtraRecipient ? (
              ...
              <div className="space-y-3 rounded-2xl border border-[#2A3441] bg-[#1B2430] p-4">
                <div className="flex items-center justify-between gap-3">
                  <label className="text-sm font-medium text-white">Additional recipient</label>
                  <button
                    type="button"
                    onClick={() => {
